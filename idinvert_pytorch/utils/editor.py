...
def mix_style(style_codes,
  ...
  if not is_style_layerwise:
    ...
    style_codes = style_codes[:, mix_layers][:, np.newaxis]
    print(style_codes.shape)
    # print(style_codes.shape)
    style_codes = np.tile(style_codes, [num_layers if axis == 1 else 1 for axis in range(style_codes.ndim)])
    print(style_codes.shape)
  if not is_content_layerwise:
    content_codes = content_codes[:, np.newaxis]
    # content_codes = content_codes[:, mix_layers][:, np.newaxis]
    content_codes = np.tile(content_codes, [num_layers if axis == 1 else 1 for axis in range(content_codes.ndim)])
